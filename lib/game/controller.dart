...
class Game {
  ...
  /// ВАЖНО: День 1 не влияет на день 2 - правило действует только со дня 2
  bool _wasVotingSkippedDueToMultipleKicks(int day) {
    // День 1 не влияет на следующий день
    if (day == 1) {
      return false;
    }
    ...
    // 1. Это был день 2 или позже (день 1 не влияет на следующий день)
    // 2. Были речи
    // 3. Не было голосования
    // 4. Было 2+ удаления
    return kicksInDay >= 2;
  }
